...
void process_student_grade(void(*func_array[3])(char*,int),char*name,int grade){

  if (grade <60){
    func_array[MAD](name,grade);
  }
  else if ((grade >60) && (grade <100)){
    func_array[NORMAL](name,grade);
  }
  else if (grade == 100){
    func_array[BEST](name,grade);
  }
...
int main()
{
  ...
  char * ptr_name = student;

  void(*func_array[3])(char*,int);

  int array_len = sizeof(func_array);

  void * ptr_array = func_array;



  func_array[MAD]= mad;
  func_array[NORMAL]= normal;
  func_array[BEST]= happy;



  process_student_grade (ptr_array,ptr_name,grade);
