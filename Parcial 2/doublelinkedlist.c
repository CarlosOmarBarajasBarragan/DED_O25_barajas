...
//Pending:
//Pending:
void printDoublelinkedlist(node *list) 
{
  node * curr = list;
...
int insertafter(int new_num, int n_search)
{
  ...
  while (n_ptr != NULL){
    if(n_ptr->value == n_search){
     ...
     n_node = newNode(new_num);
      next = n_ptr->next;
      n_ptr->next = n_node;
      n_node->next = next;

if (next != NULL) { // ✅ Solución: Solo actualiza el 'prev' si 'next' existe
          next->prev = n_node;
      }
...
void delete(int n)
{
    // 1. Corrección: Usar 'return;' en una función 'void'
    if (header == NULL){
    ...
    node * n_ptr = header;
    node * n_prev = NULL; // Usaremos esta variable para el nodo anterior

    while (n_ptr != NULL){
        if (n_ptr->value == n){
            
            // CASO 1: El nodo a eliminar es el HEAD (n_ptr es el header)
            // n_prev es NULL solo en la primera iteración cuando n_ptr == header
            if (n_prev == NULL){ 
                header = n_ptr->next;
                
                // Si la lista no queda vacía, actualizamos el nuevo header.
                if (header != NULL) {
                    header->prev = NULL;
                }
            }
            ...
            // CASO 2: El nodo a eliminar está en el medio o al final
                ...
                // Enlazar el nodo anterior (n_prev) con el nodo siguiente (n_ptr->next)
                n_prev->next = n_ptr->next;
                
                // ⚠️ Manejar el caso del ÚLTIMO NODO
                // Solo si n_ptr->next NO es NULL, actualizamos su puntero 'prev'.
                if (n_ptr->next != NULL) {
                    n_ptr->next->prev = n_prev;
                }
            }
            
            // Liberar memoria del nodo encontrado
            free(n_ptr);
            return; // Terminar la función después de la eliminación
        }
        
        // Mover punteros
        n_prev = n_ptr; // 2. Corrección: Usar 'n_prev'
        n_ptr = n_ptr->next;
    }
    
    // Si el bucle termina sin un 'return', el valor no se encontró.
