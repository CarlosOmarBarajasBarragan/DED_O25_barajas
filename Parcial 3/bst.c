...
/* prints folling the PRE-ORDER principile (P-L-R)*/
...
/* add a number VALUE into the tree, do it iteratively */
...
void learn_traversing_tree()
{
 // ====  STATIC creation of a three (from the diagram above)
  ...
  printf("\nPRINTING IN ORDER L P R (using recursion) \n");
  print_inorder(root);
  printf("\n");
  
  //print pre order (recursively)
  printf("\nPRINTING PRE ORDER P L R (using recursion) \n");
  print_preorder(root);
  printf("\n");

  //print post order (recursivo)
  printf("\nPRINTING POST ORDER L R P (using recursion) \n");
...
void print_preorder(node * n){
  ...
  print_preorder(n->left); //L

  print_preorder(n->right); //R
}
...
void print_postorder(node * n){
  ...
  print_postorder(n->left); //L

  print_postorder(n->right); //R
